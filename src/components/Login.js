...
class Login extends Component {
  ...
  loginWithGoogle () {
    firebase.auth().signInWithPopup(google).then(result => {
        ...
        // ...
        this.setState({token: result.credential.accessToken, user: result.user, isLoggedIn : true}, this.showState)
      }).catch(error => {
        // Handle Errors here.
        ...
        this.setState({credential: error.credential, email: error.email, errorCode : error.code, errorMessage: error.message, isLoggedIn: false},
        this.showState  
        )
      });
  }
